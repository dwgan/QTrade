...
- 查看市场风险、行业排名和候选股票；
- 编辑自选股并查看每日状态；
- 创建防作弊研究方案并在开发集、验证集和封存集上运行回测；
- 查看收益、回撤、Sharpe、成本和相对基准权益曲线；
- 查看后台任务进度、日志和数据质量状态。
...
```powershell
qtrade data coverage --start 2015-01-01 --end 2026-07-24
```

覆盖率检查不会访问供应商；它按交易日日历核对日频与月末分区，有缺口时返回非零退出码并显示第一个缺失日期。
